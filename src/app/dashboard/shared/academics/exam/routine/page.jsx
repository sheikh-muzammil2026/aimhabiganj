...
export default function AdminRoutineForm() {
  ...
  const [note, setNote] = useState(
    "সকল বিভাগের পরীক্ষার সময় সকাল ৯:০০ থেকে ১১:৩০ মিনিট পর্যন্ত",
  );
  ...
  const [dates, setDates] = useState([
    {
      id: "col_1",
      hijri: "1447/02/23",
      gregorian: "18/08/2025",
      day: "সোমবার",
    },
    {
      id: "col_2",
      hijri: "1447/02/24",
      gregorian: "19/08/2025",
      day: "মঙ্গলবার",
    },
  ]);
    ...
    const fetchExistingRoutine = async () => {
      ...
      try {
        ...
        if (data.success && data.data) {
          ...
          toast.info(
            "পূর্বের সেভ করা রুটিন পাওয়া গিয়েছে এবং লোড করা হয়েছে।",
          );
        }
      } catch (err) {
  ...
  const handleGregorianChange = (id, rawValue) => {
    if (!rawValue) {
      setDates((prev) =>
        prev.map((d) =>
          d.id === id
            ? { ...d, gregorian: "", gregorianRaw: "", hijri: "" }
            : d,
        ),
      );
      return;
    ...
    const formattedGregorian =
      parts.length === 3 ? `${parts[2]}/${parts[1]}/${parts[0]}` : rawValue;
  ...
  const addDateColumn = () => {
    ...
    setDates((prev) => [
      ...prev,
      { id: newId, hijri: "", gregorian: "", day: "সোমবার" },
    ]);
  };
  ...
  const handleSave = async () => {
    ...
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_SERVER_API}/api/admin/routine`,
        {
          method: "POST",
      ...
      if (data.success) {
        ...
        toast.error(
          "রুটিন সেভ করতে সমস্যা হয়েছে: " + (data.error || data.message),
        );
      }
    } catch (err) {
                    ...
                    <div className="mt-2 space-y-2">
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            handleGregorianChange(d.id, e.target.value)
                          }
                      ...
                      <div>
                        <div className="flex justify-between items-center mb-0.5">
                          <label className="block text-[10px] text-gray-500 font-bold">
                            হিজরী তারিখ:
                          </label>
                          <button
                            type="button"
                        ...
                        <input
                          ...
                          value={d.hijri}
                          onChange={(e) =>
                            updateDate(d.id, "hijri", e.target.value)
                          }
                          className="w-full border p-1 text-sm rounded bg-white"
                        />
                      </div>
                      <div>
                        ...
                        <select
                          value={d.day}
                          onChange={(e) =>
                            updateDate(d.id, "day", e.target.value)
                          }
                          className="w-full border p-1 text-sm rounded bg-white"
                        >
              ...
              <table className="w-full border-collapse text-sm">
                <thead>
                  <tr className="bg-gray-100 border-b">
                    <th className="p-2 border border-gray-300 w-48 text-left">
                      শ্রেণি
                    </th>
                    {dates.map((d, idx) => (
                      <th
                        key={d.id}
                        className="p-2 border border-gray-300 text-center min-w-[120px]"
                      >
                        <div>কলাম {idx + 1}</div>
                        <div className="text-xs text-gray-500">
                          {d.gregorian || "তারিখ নাই"}
                        </div>
                      </th>
                    ))}
                    ...
                    <tr key={cIdx} className="hover:bg-gray-50">
                      <td className="p-2 border border-gray-300">
                        <input
                          ...
                          value={cls}
                          onChange={(e) =>
                            updateClassName(cIdx, e.target.value)
                          }
                      ...
                      {dates.map((d) => (
                        <td key={d.id} className="p-1 border border-gray-300">
                          <input
                            ...
                            onChange={(e) =>
                              handleSubjectChange(cls, d.id, e.target.value)
                            }
