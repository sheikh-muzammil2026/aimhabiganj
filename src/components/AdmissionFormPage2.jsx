    ...
    <div className="w-full min-h-[11.69in] bg-white p-10 flex flex-col justify-between box-border text-gray-800 relative font-bengali print:min-h-screen">
        ...
        <div className="flex-1 mr-4">
          ...
          <div className="space-y-2 text-sm">
            <div className="flex items-center gap-2">
              ...
              <label className="flex items-center gap-1"><input type="radio" name="physicalProblem" value="হ্যাঁ" checked={formData.physicalProblem === "হ্যাঁ"} onChange={handleChange} className="accent-orange-600" /> হ্যাঁ</label>
              <label className="flex items-center gap-1"><input type="radio" name="physicalProblem" value="না" checked={formData.physicalProblem === "না"} onChange={handleChange} className="accent-orange-600" /> না</label>
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <label className="flex items-center gap-1"><input type="radio" name="cleanlinessLover" value="হ্যাঁ" checked={formData.cleanlinessLover === "হ্যাঁ"} onChange={handleChange} className="accent-orange-600" /> হ্যাঁ</label>
              <label className="flex items-center gap-1"><input type="radio" name="cleanlinessLover" value="না" checked={formData.cleanlinessLover === "না"} onChange={handleChange} className="accent-orange-600" /> না</label>
            </div>

            <div className="flex items-center gap-2">
              <span className="font-semibold text-gray-700">৩. খাবার গ্রহণে সে কি অনাগ্রহী?</span>
              <label className="flex items-center gap-1"><input type="radio" name="foodReluctance" value="হ্যাঁ" checked={formData.foodReluctance === "হ্যাঁ"} onChange={handleChange} className="accent-orange-600" /> হ্যাঁ</label>
              <label className="flex items-center gap-1"><input type="radio" name="foodReluctance" value="না" checked={formData.foodReluctance === "না"} onChange={handleChange} className="accent-orange-600" /> না</label>
            </div>

            <div className="flex items-center gap-2">
              <span className="font-semibold text-gray-700">৪. কি ধরণের খাবার খেতে বেশী পছন্দ করে?</span>
              <label className="flex items-center gap-1"><input type="radio" name="favFoodType" value="বাসায় তৈরি খাবার" checked={formData.favFoodType === "বাসায় তৈরি খাবার"} onChange={handleChange} className="accent-orange-600" /> বাসায় তৈরি খাবার</label>
              <label className="flex items-center gap-1"><input type="radio" name="favFoodType" value="ফাস্টফুড" checked={formData.favFoodType === "ফাস্টফুড"} onChange={handleChange} className="accent-orange-600" /> ফাস্টফুড</label>
            </div>

            <div className="flex items-center gap-2">
              <span className="font-semibold text-gray-700">৫. সালাত পড়তে অভ্যস্ত কি?</span>
              <label className="flex items-center gap-1"><input type="radio" name="prayerAddicted" value="হ্যাঁ" checked={formData.prayerAddicted === "হ্যাঁ"} onChange={handleChange} className="accent-orange-600" /> হ্যাঁ</label>
              <label className="flex items-center gap-1"><input type="radio" name="prayerAddicted" value="না" checked={formData.prayerAddicted === "না"} onChange={handleChange} className="accent-orange-600" /> না</label>
            </div>

            <div className="grid grid-cols-2 gap-4">
              <div className="flex items-end gap-1">
                <span className="text-gray-700 font-semibold whitespace-nowrap">৬. সাধারণত রাতে কখন ঘুমায়?</span>
                <input type="text" name="sleepTime" value={formData.sleepTime} onChange={handleChange} className="flex-1 border-b border-gray-400 text-center focus:outline-none" />
              </div>
              <div className="flex items-end gap-1">
                <span className="text-gray-700 font-semibold whitespace-nowrap">৭. সকালে কখন ঘুম থেকে উঠে?</span>
                <input type="text" name="wakeUpTime" value={formData.wakeUpTime} onChange={handleChange} className="flex-1 border-b border-gray-400 text-center focus:outline-none" />
              </div>
            </div>

            <div className="flex items-end gap-2">
              <span className="font-semibold text-gray-700 whitespace-nowrap">৮. তার প্রিয় জিনিস কী?</span>
              <input type="text" name="favThing" value={formData.favThing} onChange={handleChange} className="flex-1 border-b border-gray-400 focus:outline-none" />
            </div>

            <div className="flex items-end gap-2">
              <span className="font-semibold text-gray-700 whitespace-nowrap">৯. কোন বিষয়ে তাকে বেশি দুশ্চিন্তিত করে?</span>
              <input type="text" name="anxietyReason" value={formData.anxietyReason} onChange={handleChange} className="flex-1 border-b border-gray-400 focus:outline-none" />
            </div>
          </div>
        </div>

        {/* অভিভাবকের ছবি বক্স */}
        <div className="w-32 h-40 border-2 border-dashed border-gray-400 bg-gray-50 flex flex-col items-center justify-center p-2 text-center rounded mt-2">
          <span className="text-xs font-bold text-gray-500">অভিভাবকের ছবি</span>
        </div>
      </div>
        ...
        <div className="space-y-4 text-sm">
          ...
          <div className="border border-gray-200 p-3 rounded-md space-y-3 bg-gray-50/50">
            ...
            <div className="grid grid-cols-3 gap-4 items-center">
              <div className="flex gap-4">
                ...
                <label className="flex items-center gap-1"><input type="radio" name="fatherStatus" value="জীবিত" checked={formData.fatherStatus === "জীবিত"} onChange={handleChange} /> জীবিত</label>
                <label className="flex items-center gap-1"><input type="radio" name="fatherStatus" value="মৃত" checked={formData.fatherStatus === "মৃত"} onChange={handleChange} /> মৃত</label>
              </div>
              <div className="flex items-end gap-2">
          ...
          <div className="border border-gray-200 p-3 rounded-md space-y-3 bg-gray-50/50">
            ...
            <div className="grid grid-cols-3 gap-4 items-center">
              <div className="flex gap-4">
                <span className="font-bold text-gray-700">জীবিত/মৃত:</span>
                <label className="flex items-center gap-1"><input type="radio" name="motherStatus" value="জীবিত" checked={formData.motherStatus === "জীবিত"} onChange={handleChange} /> জীবিত</label>
                <label className="flex items-center gap-1"><input type="radio" name="motherStatus" value="মৃত" checked={formData.motherStatus === "মৃত"} onChange={handleChange} /> মৃত</label>
              </div>
              <div className="flex items-end gap-2">
                <span className="font-bold text-gray-700 whitespace-nowrap">পেশা:</span>
          ...
          <div className="border border-dashed border-gray-300 p-3 rounded-md space-y-3 bg-gray-50/20">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="flex items-end gap-2">
                ...
                <input type="text" name="guardianAnnualIncome" value={formData.guardianAnnualIncome} onChange={handleChange} className="w-24 border-b border-dotted border-gray-400 text-center focus:outline-none bg-transparent" />
                <span className="text-gray-500">কথায়:</span>
                <input type="text" name="guardianAnnualIncomeWords" value={formData.guardianAnnualIncomeWords || ""} onChange={handleChange} className="flex-1 border-b border-dotted border-gray-400 focus:outline-none bg-transparent" />
              </div>
            </div>
          </div>
      ...
      <div className="w-full mt-2">
        ...
        <div className="text-sm leading-relaxed text-gray-700 text-justify space-y-3">
          <p>
            আমি উপরে উল্লেখিত শিক্ষার্থীর <span className="inline-block border-b border-gray-400 w-24 text-center font-bold"></span> (সম্পর্ক) এবং বৈধ অভিভাবক হিসেবে অঙ্গীকার করছি যে, এই প্রতিষ্ঠানের যাবতীয় বিষয় সম্পর্কে সম্যক অবগত হয়ে আমার অভিভাবকত্বে <span className="inline-block border-b border-gray-400 w-48 text-center font-bold"></span> কে আবাসিক/অনাবাসিক/ডে-কেয়ার শিক্ষার্থী হিসেবে ভর্তি করার ইচ্ছা পোষণ করলাম।
          </p>
          <p>
        ...
        <div className="flex justify-between items-end mt-14 pt-4">
          ...
          <div className="text-center w-48 border-t border-gray-500 pt-1 text-sm font-bold text-gray-700">
            অভিভাবকের স্বাক্ষর
          </div>
        </div>
      </div>
